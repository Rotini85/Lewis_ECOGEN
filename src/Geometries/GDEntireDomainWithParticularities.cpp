...
std::string ICtype = ""; // Default value

    tinyxml2::XMLDocument doc;
    if (doc.LoadFile("/home/sdcfd/ECOGEN/rundirectory/RT/libtest/main.xml") == tinyxml2::XML_SUCCESS) {
        tinyxml2::XMLElement* icElem = doc.FirstChildElement("initialCondition");
        if (icElem && icElem->GetText()) {
            ICtype = icElem->GetText(); // Gets the string inside <initialCondition>
        }
    } else {
    ...
    std::cout << "Initial Condition type: " << ICtype << std::endl;

tinyxml2::XMLDocument doc;
if (doc.LoadFile("/home/sdcfd/ECOGEN/rundirectory/RT/libtest/main.xml") == tinyxml2::XML_SUCCESS) {
    tinyxml2::XMLElement* root = doc.FirstChildElement("testCase");  // adjust if your root element is different
    if (root) {
        tinyxml2::XMLElement* icElem = root->FirstChildElement("initialCondition");
        if (icElem && icElem->GetText()) {
            ICtype = icElem->GetText(); // Sets ICtype to the value inside <initialCondition>
        }
    }
} else {
    std::cerr << "Failed to load XML file!" << std::endl;
...
if (doc.LoadFile("/home/sdcfd/ECOGEN/rundirectory/RT/libtest/main.xml") == tinyxml2::XML_SUCCESS) {
    tinyxml2::XMLElement* icElem = doc.FirstChildElement("initialCondition");
    if (icElem && icElem->GetText()) {
        ICtype = icElem->GetText(); // Sets ICtype to the value between <initialCondition> and </initialCondition>
    }
}
